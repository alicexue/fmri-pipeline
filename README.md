...
4. Fill out condition_key.json under the 'model-\<modelname>' directory. The keys are the task names and the values are json objects with EV names as keys and the condition names as values. (Note: The EV files, *_ev-00\<N>, are always padded with leading zeros so that there are 3 digits.)
   - Example condition_key.json:
        ...
        ```
5. If you need to specify task contrasts, fill out task_contrasts.json. The keys should be the task names and the values should be json objects where the keys are the names of the contrasts and the values are lists that represent the contrast vector. If you don't want to specify task contrasts for this model, remove this json file.

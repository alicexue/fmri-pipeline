...
If anat and func folders exist under the session directory, they are copied to the directory above the session directory (the subject folder)
		...
		for d in subdir_contents:
			if d.startswith('ses-'):
				...
				if os.path.exists(anatdir):
					print "Copying contents of %s to %s"%(anatdir, subdir) # copy anat dir (under session dir) contents to sub dir anat dir
